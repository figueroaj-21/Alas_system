...
if ($existencia_actual - $cantidad_salida < $stock_minimo) {
    echo '<script>
            ...
            if (confirmacion) {
                window.location.href = "procesar_salida.php?confirmacion=1&descripcion=' . urlencode($descripcion) . '&fecha_salida=' . urlencode($fecha_salida) . '&cantidad_salida=' . urlencode($cantidad_salida) . '&id_producto=' . urlencode($id_producto) . '&factura_venta=' . urlencode($factura_venta) . '";
            } else {
    ...
    header("Location: procesar_salida.php?confirmacion=1&descripcion=" . urlencode($descripcion) . "&fecha_salida=" . urlencode($fecha_salida) . "&cantidad_salida=" . urlencode($cantidad_salida) . "&id_producto=" . urlencode($id_producto) . "&factura_venta=" . urlencode($factura_venta));
    exit;
}
?>
