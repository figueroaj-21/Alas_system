...
$sql_facturas = "SELECT e.id_entrada, e.fecha_entrada, e.cantidad_entrada, e.factura_compra, e.id_producto, p.descripcion FROM tbl_entrada e 
   JOIN tbl_productos p ON e.id_producto = p.id_producto 
   WHERE e.fecha_entrada BETWEEN '$fecha_inicio' AND '$fecha_fin'";

// Ejecutar la consulta y obtener los resultados
$resultado = mysqli_query($conexion, $sql_facturas) or die("Error al consultar Entradas: " . mysqli_error($conexion));
...
$pdf->Write(5, utf8_decode('Reporte de Entradas por Periodo al ' . $fecha));
$pdf->Ln(25);
...
$pdf->SetTextColor(255, 255, 255);
$pdf->Cell(60, 10, utf8_decode('Fecha de Entrada'), 1, 0, 'C', 1);
$pdf->Cell(60, 10, utf8_decode('Cantidad de Entrada'), 1 , 0, 'C', 1);
...
while ($row_factura = mysqli_fetch_array($resultado, MYSQLI_ASSOC)) {
    $pdf->Cell(60, 10, $row_factura['fecha_entrada'], 1, 0, 'C', 1);
    $pdf->Cell(60, 10, $row_factura['cantidad_entrada'], 1, 0, 'C', 1);
    $pdf->Cell(60, 10, $row_factura['descripcion'], 1, 0, 'C', 1);
    $pdf->Cell(60, 10, $row_factura['factura_compra'], 1, 0, 'C', 1);
    $pdf->Ln();
}
